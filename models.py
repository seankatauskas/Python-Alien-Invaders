...
class Ship(GSprite):
    ...
    def __init__(self,x):
        ...
        super().__init__(x=x,y=SHIP_BOTTOM+(SHIP_HEIGHT/2),source=SHIP_IMAGE,
                            width=SHIP_WIDTH,height=SHIP_HEIGHT,format=(2,4))
    ...
    def collides(self,bolt):
        ...
        """
        def contain(self,bolt):
            x = False
            if self.contains([bolt.left,bolt.top]):
                return True
            if self.contains([bolt.right,bolt.top]):
                return True
            if self.contains([bolt.left,bolt.bottom]):
                return True
            if self.contains([bolt.right,bolt.bottom]):
                return True
            return x


        if not bolt.isPlayerBolt() and contain(self,bolt):
            return True
        else:
            return False
...
class Alien(GImage):
    ...
    """
    #  IF YOU ADD ATTRIBUTES, LIST THEM BELOW
    #Attribute width: width dimension of alien
    #Invariant: width is an int >= 0

    #Attribute height: height dimension of alien
    #Invariant: height is an int >= 0

    #Attribute source: the image files for the aliens (bottom to top)
    #Invariant: source is an image file
    ...
    def collides(self,bolt):
        ...
        if bolt.isPlayerBolt() and contain(self,bolt):
            return True
        else:
            return False
...
class Bolt(GRectangle):
    ...
    def __init__(self,x,y,velocity):
        super().__init__(x=x,y=y,fillcolor="black",linecolor="black",
                                            width=BOLT_WIDTH,height=BOLT_HEIGHT)
        self._velocity = velocity

    def isPlayerBolt(self):
        """
        return a boolean for player bolt velocity
        """
        return self.velocity==BOLT_SPEED
