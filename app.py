...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        if self._state== STATE_INACTIVE:
            self._text = GLabel(text="Press 'S' to Play",font_size=30,
                                ...
                                linecolor=[0.5,1,0.5,1],y=GAME_HEIGHT/2)
            self._wave=None
        else:
            self._text = None
    ...
    def update(self,dt):
        ...
        if self._state == STATE_CONTINUE:
            self._wave.ship = Ship(GAME_WIDTH/2)
        ...
        if self._state== STATE_PAUSED and self._wave.lives!=0:
            if self._wave.lives!=1:
                self._text = GLabel(text="Press 'S' to Continue",font_size=30,
                                    font_name="RetroGame",x=GAME_WIDTH/2,
                                    linecolor=[0,0,1,1],y=GAME_HEIGHT/2)
    ...
    def _detcomplete(self):
        if self._wave!= None:
            if self._wave.lives==0:
                ...
                self._text = GLabel(text="YOU LOSE - GAME OVER",font_size=30,
                                    font_name="RetroGame",x=GAME_WIDTH/2,
                                    linecolor=[0,0,0,1],y=GAME_HEIGHT/2)

            elif self._wave.dead==True:
                self._state = STATE_COMPLETE
                self._text = GLabel(text="YOU WIN - GAME OVER",font_size=30,
                                    font_name="RetroGame",x=GAME_WIDTH/2,
                                    linecolor=[0,1,0,1],y=GAME_HEIGHT/2)
            elif self._wave.line==True:
                self._state = STATE_COMPLETE
                self._text = GLabel(text="YOU LOSE - GAME OVER",font_size=30,
                                    font_name="RetroGame",x=GAME_WIDTH/2,
                                    linecolor=[0,0,0,1],y=GAME_HEIGHT/2)
    ...
    def _detpause(self):
        ...
        determine if paused
        """
        if self._state==STATE_ACTIVE and self._wave.ship == None:
            self._state = STATE_PAUSED

    def _detstart(self):
        # Determine the current number of keys pressed
        curr_keys = self.input.key_count
        spc_key = self.input.is_key_down('s')

        # Only change if we have just pressed the keys this animation frame
        change = curr_keys > 0 and self._lastkeys == 0 and spc_key == True
    ...
    def _activatewave(self):
        """
        removes welcome screen, creates and assign wave object, changes
    ...
    def _determineStart(self):
        """
        Determines if any key has been presed while in STATE_INACTIVE
        """
        # Determine the current number of keys pressed
        curr_keys = self.input.key_count
        spc_key = self.input.is_key_down('s')
